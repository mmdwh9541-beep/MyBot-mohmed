...
async function executeTrade(symbol, side, amountOrQty) {
    ...
    if (side === 'BUY') {
        params.quoteOrderQty = amountOrQty.toFixed(2).toString(); 
    } else {
        params.quantity = formatQuantity(symbol, amountOrQty); 
    }
    ...
    if (decision === 'BUY' && !activePositions[symbol]) {
        ...
        if (parseFloat(liveWalletBalance) < dynamicTradeAmount) {
            if (parseFloat(liveWalletBalance) > 10) { 
                dynamicTradeAmount = parseFloat(liveWalletBalance) * 0.98; 
            } else {
                return 'NO_BALANCE'; 
            }
        }
        ...
        if (orderResult && orderResult.status === 'FILLED') {
            ...
            sendTelegramMessage(`🟢 <b>Buy Executed (Dynamic Sizing)</b>\n<b>Symbol:</b> ${symbol}\n<b>Amount Used:</b> $${dynamicTradeAmount.toFixed(2)}\n<b>Entry:</b> $${entryPrice.toFixed(4)}\n<b>TP:</b> $${activePositions[symbol].takeProfit.toFixed(4)}\n<b>SL:</b> $${activePositions[symbol].stopLoss.toFixed(4)}`);
            updateWalletBalance(); 
            return 'BOUGHT';
        }
    }
    ...
    if (activePositions[symbol]) {
        ...
        if (hitSL || hitTP) {
            ...
            if (orderResult && (orderResult.status === 'FILLED' || orderResult.status === 'NEW')) {
                ...
                sendTelegramMessage(`🔴 <b>Sell Executed</b>\n<b>Symbol:</b> ${symbol}\n<b>Result:</b> ${emoji} ($${profitUSDT.toFixed(2)})`);
                ...
                if (dailyPnL <= -maxLossAllowed && !tradingPaused) {
                    tradingPaused = true;
                    sendTelegramMessage(`🛑 <b>Emergency Trading Pause!</b>\nDaily loss reached (${dailyPnL.toFixed(2)}$) representing 10% of total equity.`);
                }
...
app.post('/api/emergency-close', async (req, res) => {
    ...
    if (symbolsToClose.length === 0) {
        return res.json({ success: false, msg: 'No open positions currently.' });
    }

    sendTelegramMessage(`🚨 <b>Emergency close initiated for ${symbolsToClose.length} positions!</b>`);
    ...
    updateWalletBalance();
    sendTelegramMessage(`✅ <b>Emergency close completed!</b> Closed ${closedCount} positions.`);
    res.json({ success: true, msg: `Successfully closed ${closedCount} positions.` });
});
